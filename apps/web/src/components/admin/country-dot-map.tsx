...
function useLocalizedCountries(countries: CountryDatum[]) {
  const { t, locale } = useT()
  return useMemo(() => {
...
function CountryMapView({
  ...
  const pins = useMemo(() => {
    ...
    const out: ({ x: number; y: number } & PinData)[] = []
    for (const c of localizedCountries) {
      const centroid = COUNTRY_CENTROIDS[c.code]
      if (!centroid) continue
      // d3-geo expects [lng, lat].
      const xy = PIN_PROJECTION([centroid[1], centroid[0]])
  ...
  const pinTooltip = useCallback(
    (pin: { x: number; y: number } & PinData): PinTooltip => ({
      ...pin,
      xPct: (pin.x / MAP_WIDTH) * 100,
      yPct: (pin.y / MAP_HEIGHT) * 100,
    }),
    []
  )
