/** 一次性脚本：把中英双语部署指南系列插入正式环境 Turso 库。
 *  用法：TURSO_PROD_TOKEN=<token> node scripts/seed-prod-guides.mjs
 *  幂等：已存在的 slug 先删除再插入（重复运行安全）。 */
...
const DB_URL =
  process.env.TURSO_PROD_URL ??
  "https://bitlog-zephyr110.aws-ap-northeast-1.turso.io"
const TOKEN = process.env.TURSO_PROD_TOKEN ?? ""
...
const today =
  process.env.TURSO_PROD_DATE ?? new Date().toISOString().slice(0, 10)
...
for (const g of guides) {
  ...
  console.log(`queued ${g.slug} (${wordCount} words)`)
}
...
if (errors.length) {
  console.error("FAILED:", JSON.stringify(errors[0]).slice(0, 300))
  process.exit(1)
}
